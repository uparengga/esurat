...
$excelku->getActiveSheet()->setCellValue('A2', "DINAS KEPENDUDUKAN DAN PENCATATAN SIPIL KABUPATEN BADUNG");
$excelku->getActiveSheet()->mergeCells('A3:H3');
$excelku->getActiveSheet()->setCellValue('A3', "BIDANG PIAK & PEANFAATAN DATA");
// $excelku->getActiveSheet()->mergeCells('A4:H4');
// $excelku->getActiveSheet()->setCellValue('A4', "USER ".$_SESSION['nama']."");
$excelku->getActiveSheet()->mergeCells('A4:H6');
$excelku->getActiveSheet()->setCellValue('A4', "Jl. Raya Sempidi Mangupura, Sempidi, Kec. Mengwi, Kabupaten Badung, Bali 80351 ");
$excelku->getActiveSheet()->mergeCells('A5:H6');
$excelku->getActiveSheet()->setCellValue('A5', "DATA SURAT MASUK BULAN $bulan TAHUN $tahun");
...
while ($data = $query1->fetch_assoc()) {
  ...
  $SI->setCellValue("A".$baris,$data['pengirim']); 
  $SI->setCellValue("B".$baris,$data['nomor_suratmasuk']); 
  $SI->setCellValue("C".$baris,$data['tanggalsurat_suratmasuk']); 
  $SI->setCellValue("D".$baris,$data['perihal_suratmasuk']); 
  $SI->setCellValue("E".$baris,$data['tanggalmasuk_suratmasuk']); 
  $SI->setCellValue("F".$baris,$data['kode_suratmasuk']); 
  $SI->setCellValue("G".$baris,$data['disposisi1']); 
  $SI->setCellValue("H".$baris,$data['tanggal_disposisi1']); 
  $SI->setCellValue("I".$baris,$data['disposisi2']); 
  $SI->setCellValue("J".$baris,$data['tanggal_disposisi2']); 
  $SI->setCellValue("K".$baris,$data['disposisi3']); 
  $SI->setCellValue("L".$baris,$data['tanggal_disposisi3']); 
